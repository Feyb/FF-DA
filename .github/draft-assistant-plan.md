...
- Init Angular workspace using Angular CLI: `ng new draft-assistant --no-create-application --package-manager pnpm --style scss`
- Generate the app: `ng generate application frontend --routing --style scss` inside the workspace, then move to `apps/draft-assistant/frontend/`.
- Add Angular Material v3: `ng add @angular/material` and configure a custom M3 theme in `packages/theme`.
- Add TailwindCSS: `npm install -D tailwindcss postcss autoprefixer` + `npx tailwindcss init`; extract shared config to `packages/tailwind-config`; use `@apply` directive in CSS files for >5 utility classes.
