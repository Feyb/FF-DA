...
export class DraftControlsCardComponent implements OnInit {
  ...
  protected onSourceModeChange(mode: DraftSourceMode): void {
    this.mockDraftUrlError = null;
    if (this.activeSourceMode() === mode) return;
  ...
  private buildLeagueDraftOption(draft: SleeperDraft): DraftOptionView {
    ...
    if (userPosition !== null) meta.push(`Pick ${userPosition.label}`);
    if (startTime) meta.push(startTime);
